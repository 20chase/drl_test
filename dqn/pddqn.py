...
class PrioritizedDoubleDQN(object):
    def __init__(self, session, args, obs_dim, act_dim):
        ...
        self.model = [self.eval_network, self.target_network]
    ...
    def _build_training_method(self):
        mean_q = tf.reduce_mean(self.eval_q)
        ...
        eval_vars = self.eval_network.all_params
        target_vars = self.target_network.all_params
    ...
    def train(self):
        ...
        # compute rets
        q_eval, q_target = self.sess.run([self.eval_q, self.target_q],
    ...
    def update_target_tl(self):
        tl.files.assign_params(self.sess, self.eval_network.all_params, self.target_network)

    def action(self, obs, test=False):
        obs = np.reshape(obs, (1, self.obs_dim))
        feed_dict =  {self.obs_ph: obs}
        if self.args.test_alg or test:
            return np.argmax(self.sess.run(self.eval_q, feed_dict)[0])
